...
from facefusion.filesystem import list_directory, get_temp_frame_paths, create_temp, move_temp, clear_temp, is_image, \
    is_video, filter_audio_paths, resolve_relative_path
...
def apply_args_new(source_path, target_path, output_path, provider, detector_score, mask_blur, skip_nsfw,
                   landmarker_score, enable_swapper, enable_face_restore) -> None:
    # general
    ...
    if enable_swapper:
        facefusion.globals.frame_processors.append('face_swapper')
...
def run_new(source_path, target_path, output_path, provider="cpu", detector_score=0.6, mask_blur=0.3, skip_nsfw=True,
            landmarker_score=0.5, enable_swapper=True, enable_face_restore=True):
    apply_args_new(source_path, target_path, output_path, provider, detector_score, mask_blur, skip_nsfw,
                   landmarker_score, enable_swapper, enable_face_restore)
